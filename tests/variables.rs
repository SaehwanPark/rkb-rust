...
fn extracts_only_candidates_with_definition_evidence() {
  ...
  assert_eq!(
    rows[0].definition,
    "Beneficiary identifier, also known as beneficiary id, 2020"
  );
...
fn writes_variable_and_canonical_artifacts_with_resolved_citations() {
  ...
  assert_eq!(
    result.canonical_variables[0].variable_label,
  ...
  assert_eq!(
    result.variables[0].source_document,
    variable_page.display().to_string()
  );
  ...
  assert!(
    edges_csv
      .starts_with("source_id,target_id,relationship,source_url,source_document,page,chunk_id\n")
  );
...
fn command_reports_partial_failures_after_writing_outputs() {
  ...
  assert!(
    result
      .unwrap_err()
      .to_string()
      .contains("variable extraction completed with 1 failures")
  );
